...
export default function ProfileSection({ profile, skills = [] }: Props) {
  ...
  const uxSkills = safeSkills.filter((s) => s.category === "ux");
  const designTools = safeSkills.filter((s) => s.category === "design");
  const techStack = safeSkills.filter((s) => s.category === "tech");
    ...
    <section className="grid grid-cols-1 md:grid-cols-12 gap-6 mb-12 animate-enter">
      ...
      >
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-1.5 relative z-10">
              <div className="relative flex h-1.5 w-1.5 items-center justify-center shrink-0">
                ...
                />
                <span className="relative inline-flex h-1.5 w-1.5 rounded-full bg-white shadow-sm"></span>
              </div>
          ...
          <a
            href="https://google.com/maps?q=Tangerang,Indonesia"
            target="_blank"
        ...
        <motion.div variants={itemVariants} className="flex-1 relative z-10 mb-8">
          <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/40 mb-2">
            About Me
          </h3>
      ...
      <div className="md:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-6 h-full content-start">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-10 gap-y-6">
            {uxSkills.map((skill, index) => (
              <div key={skill.id} className="group">
                ...
                <p className="text-[10px] text-muted-foreground mt-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform translate-y-1 group-hover:translate-y-0">
                  Level: {skill.level}
                </p>
              </div>
            ))}
          ...
          <div className="flex flex-wrap gap-2 content-start">
            {designTools.map((t) => (
              <motion.span
                ...
                whileTap={{ scale: 0.95 }}
                className="badge cursor-default bg-purple-500/5 text-purple-600 dark:text-purple-300 border-purple-100 dark:border-purple-900/50 hover:bg-purple-500 hover:text-white hover:border-purple-500 transition-colors duration-300"
              >
                {t.name}
              </motion.span>
            ))}
        ...
        >
          <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-4 flex items-center gap-2">
            <span className="text-blue-500 text-lg leading-none">●</span> Logic
            Core
          </h3>
          <div className="flex flex-wrap gap-2 content-start">
            {techStack.map((t) => (
              <motion.span
                ...
                whileHover={{ scale: 1.05, y: -2 }}
                whileTap={{ scale: 0.95 }}
                className="badge cursor-default bg-blue-500/5 text-blue-600 dark:text-blue-300 border-blue-100 dark:border-blue-900/50 hover:bg-blue-500 hover:text-white hover:border-blue-500 transition-colors duration-300"
              >
                {t.name}
              </motion.span>
            ))}
